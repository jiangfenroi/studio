...
- **环境变量**: 必须设置 `NODE_SKIP_PLATFORM_CHECK=1`（提供的 `.bat` 脚本已包含）。
...
4. (可选) 打包桌面壳：使用 Nativefier 封装 `http://localhost:9002`。

### 第二步：离线迁移（无网环境）
1. 将包含 `node_modules`、`.next`、`package.json`、`public` 及其根目录下所有文件的整个文件夹拷贝到 U盘。
2. 拷贝至无网电脑的 `D:\HealthApp` 目录下。

### 第三步：一键启动 (Intranet Server)
- **双击根目录下的 `start-app.bat`**：
  - 它会自动处理 Win7 兼容性。
  - 会在 `9002` 端口启动服务，并监听 **0.0.0.0**（所有网卡）。

## 3. 内网多终端互联 (Important!)
如果您的服务器电脑 IP 是 `172.17.126.18`：
- **本地访问**: 在该电脑上访问 `http://localhost:9002`。
- **内网他机访问**: 同一局域网内的其他电脑，只需在浏览器输入 `http://172.17.126.18:9002` 即可直接使用系统。
- **防火墙配置**: 请确保服务器电脑的防火墙已 **“入站规则”** 中放行 TCP **9002** 端口，否则其他电脑将无法连接。
...
## 5. 逻辑安全设计
- **自动注销**: 采用会话级持久化。关闭浏览器或重启电脑后，登录状态立即销毁。
- **无后台残留**: 关闭 `start-app.bat` 运行窗口后，相关 Node 进程将彻底释放内存，杜绝卡顿。
