...
- **内网互联**: 在 IP 为 `172.17.126.18` 的电脑启动后，其他内网电脑通过浏览器输入 `http://172.17.126.18:9002` 即可直接使用，无需安装任何插件。
...
## 4. MySQL 数据库初始化
请在内网中心服务器上创建 `meditrack_db`，并初始化以下核心表结构：
...
## 5. 逻辑安全与性能设计
- **自动注销**: 采用会话级持久化。关闭程序命令行窗口或重启电脑后，登录状态立即销毁，保障临床权限安全。
- **无后台残留**: 关闭运行窗口后，所有服务进程将彻底释放，杜绝后台静默运行导致的系统卡顿。
