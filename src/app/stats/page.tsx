...
export default function StatsPage() {
  ...
  const handleExport = () => {
    ...
    link.href = url
    link.download = `临床重要异常结果统计_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
    
    toast({ title: "报表生成完毕", description: `成功导出 ${filteredData.length} 条复合临床记录。` })
  }
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-primary">全量业务导出中心</h1>
          <p className="text-muted-foreground font-medium">MySQL 核心链路：支持三表实时关联大宽表预览</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={handleExport} className="gap-2 h-11 px-8 shadow-lg bg-green-600 hover:bg-green-700" disabled={filteredData.length === 0}>
            <FileSpreadsheet className="size-5" /> 导出自定义 CSV
          </Button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <Card className="lg:col-span-1 border-none shadow-md h-fit">
          <CardHeader className="bg-primary/5 py-4 flex flex-row items-center justify-between">
            <CardTitle className="text-sm flex items-center gap-2"><Filter className="size-4" /> 报表列选择</CardTitle>
              ...
              {Object.entries(COLUMNS).map(([table, cols]) => (
                <div key={table} className="mb-8">
                  <h3 className="text-[10px] font-black text-primary/60 uppercase mb-3 tracking-widest border-b pb-1">
                    {table === 'SP_PERSON' ? '个人档案字段' : table === 'SP_YCJG' ? '临床异常字段' : '随访结果字段'}
                  </h3>
          ...
          <div className="p-4 border-b flex justify-between items-center bg-muted/20">
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <Input placeholder="筛选预览内容..." className="w-64 h-8 pl-8 text-xs" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
            </div>
            <p className="text-[10px] text-muted-foreground">已选 {selectedCols.length} 个字段</p>
          </div>
          <CardContent className="p-0 flex-1 overflow-hidden">
            <ScrollArea className="h-[650px]">
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    <TableRow><TableCell colSpan={selectedCols.length} className="text-center py-20"><Loader2 className="animate-spin mx-auto mb-2" /> 正在进行三表实时联查...</TableCell></TableRow>
                  ) : filteredData.length === 0 ? (
                    <TableRow><TableCell colSpan={selectedCols.length} className="text-center py-20 text-muted-foreground">未检索到匹配的业务数据</TableCell></TableRow>
